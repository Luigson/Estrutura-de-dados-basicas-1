...
- **Busca Binária:**
  ...
  - Retorne o índice se a chave for encontrada ou `-1` se não for.
  - Para implementar a versão recursiva da busca binária, é necessário ter uma função de busca que recebe o array, a chave buscada e os índices início e fim, que serão usados para ir ajustando o espaço de busca ao longo das chamadas recursivas. Para implementar esta versão recursiva, crie uma função auxiliar da seguinte forma:

```c++
int busca_bin(const int* arr, int inicio, int fim, int alvo)
{
  // Implemente aqui sua função recursiva de busca binária
}

int busca(const int* arr, int tamanho, int alvo)
{
  return busca_bin(arr, 0, tamanho-1, alvo);
}
```

Ou seja, no código anterior, a função `busca` chama a função auxiliar `busca_bin`, que por sua vez é responsável por implementar a busca binária recursivamente.
