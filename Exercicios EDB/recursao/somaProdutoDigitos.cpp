...
std::pair<int, int> somaProdutorioDigitos(int N) {
    // Caso base: se N for um único dígito, o somatório é o próprio dígito e o produtório é o próprio dígito
    if (N < 10) {
    ...
    // Atualiza o somatório e o produtório com base no último dígito
    resultado.first += ultimoDigito;   // Soma dos dígitos
    resultado.second *= ultimoDigito;  // Produtório dos dígitos
    return resultado;
}
